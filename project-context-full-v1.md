...
description: >
  ...
  2. Send a SCOPE HINT: "surface" | "block" | "token".
  3. Include domain and user_type if inferable.
  4. DECOMPOSE the intent into WORKFLOWS — bounded UI sections as { id, intent, region? }.
...
component_type: row
level: composite
npm_path: @innovaccer/ui-assets/block-composites/ActivityLogRow
...
component_type: form
level: composite
npm_path: @innovaccer/ui-assets/block-composites/AssessmentTab
...
level: composite
npm_path: @innovaccer/ui-assets/block-composites/ChatQuickActionChip
...
level: composite
npm_path: @innovaccer/ui-assets/block-composites/EntityContextHeader
...
component_type: row
level: composite
npm_path: @innovaccer/ui-assets/block-composites/EntityRow
structural_family: actionable-list-row
structural_role: "list-row"
family_invariants:
...
component_type: card
level: composite
npm_path: @innovaccer/ui-assets/block-composites/InlineEntityCard
...
type: surface
npm_path: @innovaccer/ui-assets/surfaces/ClinicalAssessment
...
regions:
  - slot: header
    ...
    props_hint: "entityName={patient.lastFirst} primaryLabel='MRN' primaryValue={patient.mrn} secondaryLabel='DOB' secondaryValue={patient.dob}"
  ...
  - slot: safety-alert
    ...
    props_hint: "severity='high' title='Review Required' body='A self-harm indicator question received a non-zero response.' onAcknowledge={handleReviewAcknowledge}"
    layout_hint: "flex flex-col w-full"

  - slot: instrument-selector
    block_id: Select
    props_hint: "label='Instrument' placeholder='Select screening instrument' options=['PHQ-9','GAD-7','SDOH','CAGE'] required={true}"
    layout_hint: "flex flex-col gap-2 px-6 py-4 border-b border-border"

  - slot: questions
    block_id: AssessmentTab
    props_hint: "instrument={selectedInstrument} patient={patient} onComplete={handleAssessmentComplete}"
    layout_hint: "flex flex-col flex-1 overflow-y-auto px-6 py-4"

  - slot: submit-bar
    block_id: Button
    props_hint: "variant='primary' label='Record Assessment' disabled={!allAnswered} size='default'"
  ...
  - slot: loading
    block_id: Skeleton
    props_hint: "variant='card' count={9}"
...
id: IntakeForm
type: surface
npm_path: @innovaccer/ui-assets/surfaces/IntakeForm
...
id: MedicalCoder
type: surface
npm_path: @innovaccer/ui-assets/surfaces/MedicalCoder
user_type: [coordinator]

intent: >
  Three-panel workspace for medical coders managing insurance claims.
  Left panel holds a thread-based navigation history for different claim worklists.
  Center panel is an AI chat assistant that adapts its context to what's open in the canvas.
  Right panel is the main canvas showing the home dashboard, claim worklists, or claim detail pages with tabs.
  The primary workflow is: review metrics on home, open a worklist via chat prompt, drill into a claim, resolve issues through AI-suggested fixes.

what_it_omits:
  - item: Clinical notes and patient health records
    reason: "Different role and access control. Medical coders work with billing codes and payer data — clinical context belongs in the EHR, not the coding workspace."
  - item: Bulk claim actions without confirmation
    reason: "Billing modifications affect revenue and compliance. Bulk changes require explicit per-claim review."
  - item: Resolved and closed claims mixed into active worklists
    reason: "Closed claims are historical record. The worklist is an action surface — mixing closed items degrades scannability and creates false urgency."

empty_state_meaning: >
  All claims are coded correctly and no action is required. This is a positive billing signal — not an error state.

ordering: >
  Claims sorted by urgency: denial risk first, then dollar amount, then days open.
  Never alphabetical. Never recency alone. Order encodes the cost of inaction.

actions:
  - label: "Apply Suggested Code"
    stage: claim detail — chat panel CTA
    constraint: "Updates claim status to Ready to Submit. Requires AI resolution suggestion to be present."
  - label: "Submit Claim"
    stage: claim detail — after fix applied
    constraint: "Sends claim to payer. Status updates to Submitted. One step, no additional confirmation for single claim."
  - label: "Open Worklist"
    stage: home chat prompt chips
    constraint: "Creates a new thread in left nav, loads worklist in main canvas. Does not navigate away from home until thread is created."
  - label: "Open Claim Detail"
    stage: worklist row click
    constraint: "Opens new tab within current thread canvas. Does not replace worklist tab."

never:
  - Replace the worklist tab when opening a claim — always add a new tab within the same thread
  - Dismiss or hide the chat panel — it is always visible and context-aware
  - Show a generic confirmation dialog for single-claim resolution actions
  - Mix different thread contexts — each thread is isolated to its worklist and opened claims
  - Use the home canvas to show worklist data — home is metrics only
```
...
id: OutreachLog
type: surface
npm_path: @innovaccer/ui-assets/surfaces/OutreachLog
user_type: [coordinator]
...
regions:
  - slot: toolbar
    block_id: SectionHeader
    props_hint: "label='OUTREACH LOG — LAST 24 HOURS' count={totalCount}"
    layout_hint: "flex flex-row items-center justify-between px-4 py-2 sticky top-0 z-10 bg-background border-b border-border"

  - slot: list
    block_id: ActivityLogRow
    props_hint: "entityName={entry.patientName} activityType={entry.outreachType} outcome={entry.outcome} actor={entry.coordinatorName} timestamp={entry.loggedAt} flagged={entry.outcome === 'no-answer'}"
    layout_hint: "flex flex-col divide-y divide-border/40 overflow-y-auto flex-1"

  - slot: empty-state
    block_id: Alert
    props_hint: "variant='default' title='No outreach logged in the last 24 hours.'"
    layout_hint: "flex items-center justify-center p-8"

  - slot: loading
    block_id: Skeleton
    props_hint: "variant='text' count={8}"
...
id: Worklist
type: surface
npm_path: @innovaccer/ui-assets/surfaces/Worklist
user_type: [coordinator]
