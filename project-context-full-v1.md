...
> Auto-generated from repo on 2026-04-01. Do not edit manually — run `node scripts/generate-context.js` to refresh.
...
- `styling-tokens` — confidence 0.97 — v3.0.0 — token definitions removed from genome. Single rule: use tokens from @innovaccer/ui-assets/tokens. Token definitions live in the package.
...
- **Borders over shadows.** A thin border in `border-border` communicates "this is a region" without adding visual weight. When even a border feels heavy. Reserve heavy borders for edges that need structural definition.
...
| Range | Character | When to use |
|---|---|---|
| 1–2 | **Near-static.** Hover and focus state changes only. Panel open/close transitions are fast and unobtrusive. Nothing that loops, nothing that staggers, nothing that draws attention to the interface itself. | The default for all product screens. Forms, tables, dashboards, detail views, settings. Motion is invisible infrastructure — it confirms that a click registered and that a panel came from somewhere. That's it. |
...
| 6–7 | **Productive density.** Tighter spacing, more information per viewport. Hierarchy is critical — density without structure is chaos. This is the working range for screens where throughput matters. | Dashboards, list views, multi-panel layouts, workflow screens. The screen has multiple data types, actions, and status indicators coexisting. Use spacing tokens from `@innovaccer/ui-assets/tokens`. |
| 8–9 | **Compact and utilitarian.** Reduced padding, compact row heights, monospace numbers in data columns. Dividers and borders become essential structural tools — `divide-y` and thin separators replace whitespace-based grouping. | Data tables, monitoring views, admin consoles, audit logs — any screen where the volume of information is the point. The user's task is to scan, compare, or process at speed. |
...
- **Weight hierarchy is strict.** Three weights, three roles — no exceptions:
  - **Regular** — the workhorse. Body text, interactive controls (buttons, inputs, selects, toggles), table cells, list content, descriptions, helper text. This is the default weight for the entire UI.
  - **Medium** — structural emphasis. Section headings, column headers, active navigation items, form labels, stat labels. Used to create scannable landmarks within a page without shouting.
  - **Semibold** — rare, high-level titles only. Page titles, dialog titles, card titles when the card is the primary content. If you're reaching for semibold on anything that isn't a title, reconsider — size and color contrast should do the job instead.
- **Hierarchy comes from size and color, not weight stacking.** A regular-weight label in `text-muted-foreground` against a regular-weight value in `text-foreground` at a larger size creates clear hierarchy without touching font-weight. Resist the urge to make things bolder — make them bigger, darker, or lighter instead.
...
Color is a language. Every hue in the interface is a word — use too many and the sentence is noise. Use color tokens from `@innovaccer/ui-assets/tokens`. Do not introduce colors outside the token system.
...
- **Use tokens from `@innovaccer/ui-assets/tokens`.** Semantic tokens encode meaning that raw hex values don't — using them consistently is how the interface stays coherent across dozens of screens built at different times.
...
- **Rhythm over randomness.** Consistent use of spacing tokens from `@innovaccer/ui-assets/tokens` — applied religiously — creates the subconscious feeling of "one system." Irregular spacing, even by a few pixels, registers as carelessness.
...
- **Speed is a feature.** Micro-interactions (hover, focus, press feedback) should be nearly instant. Component state changes (dropdown open, toggle flip) should be fast. Layout shifts are the only case for slower transitions. Anything that makes the user wait for an animation to finish is borrowing time they didn't offer.
- **Press feedback is physical.** Interactive elements scale down subtly on press to give tactile confirmation. This replaces shadow changes or color darkening as the primary press signal. The scale is small enough to feel natural, never bouncy or exaggerated.
...
- **Border radius consistency.** Use border radius tokens from `@innovaccer/ui-assets/tokens` consistently — mixing values across a screen reads as indecisive, even if no one can pinpoint why.
...
Use tokens from `@innovaccer/ui-assets/tokens`. Do not use hardcoded hex values, CSS color functions (rgba, oklch, hsla), raw Tailwind color utilities (e.g. `bg-red-500`), or arbitrary spacing values (e.g. `gap-[10px]`). All color, spacing, typography, and elevation values must come from the design token system exported by `@innovaccer/ui-assets/tokens`.
...
summary: >
  Inline contextual message for non-critical feedback. presents informational
  or warning-level messages within the page flow.
...
variants:
  access: use for access-related messages such as permissions, entitlements, or restricted content
  destructive: use for error states, failed actions, or destructive outcomes requiring user attention
  warning: use for cautionary messages where action may lead to unintended consequences
  alert: use for important notices that require acknowledgment but are not errors
...
embedding_hint: >
  alert info warning feedback
  contextual notice tip guidance
```
...
family_invariants:
  ...
  - "all visual decisions flow from Alert Banner Block"
  - "If actions are present, Critical and High must render Acknowledge — never Dismiss on Critical"
...
summary: >
  ...
  attention or action. Permitted actions are determined by severity level.
  This component is the most safety-critical
  in the library.
...
embedding_hint: >
  ...
  warning notification required action
```
...
family_invariants:
  - "Badge content should never wrap into more than 1 line"
confidence: 0.85
version: 2.0.0

summary: >
  Compact label for displaying status, category, or count information.
  Color is never the sole differentiator; text label is always present.
...
family_invariants:
  ...
  - "font-light (300 weight) for all variants except link; font-normal (400) for link"
  - "transition-all inherited from shadcn base; no explicit transition override in block"
confidence: 0.85
version: 3.0.0

summary: >
  Primary interactive trigger for user actions. Linear-inspired flat aesthetic
  with font-weight 300 (light) for standard variants and 400 (normal) for the
  link variant, no shadows, tight padding, and no scale transform on press.
  Supports six visual types (primary, destructive, basic, outline,
  ...
  asChild render delegation, and five interactive states (default, hover,
  pressed, focused, disabled). Icon-only and lg size are not available for
  the link type.
...
variants:
  primary: bg-primary text-primary-foreground, flat fill, single primary CTA per surface
  destructive: bg-destructive text-destructive-foreground, flat fill, delete and remove actions only
  basic: bg-muted text-foreground, subtle tinted fill for secondary actions
  outline: border-border bg-transparent text-foreground — also used as a toggle (pair with aria-pressed and selected state)
  transparent: bg-transparent text-foreground/70 resting, inline or toolbar actions
  link: text-primary underline-offset-2, underlines on hover, in-text navigational triggers; lg size not supported
...
content_variants:
  ...
  as-child: render as a child element such as an anchor (asChild prop)

states:
  default: resting state, flat appearance
  hover: explicit token-based bg shift — primary uses bg-primary-hover; basic/outline/transparent use bg-muted-hover; link uses text-primary-hover + underline
  pressed: active color token shift — bg-primary-active / bg-muted-active / text-primary-active; no scale transform
  focused: focus-visible:ring-2 ring-offset-1 with ring-ring token for all types except destructive which uses ring-ring-destructive
  disabled: opacity-50, pointer-events-none
  selected: outline variant with aria-pressed=true — border-primary bg-accent text-accent-foreground indicates the on state; this is the toggle pattern
...
key_rules:
  ...
  - focus ring uses ring-ring token for all types except destructive which uses ring-ring-destructive
  - link variant does not support size lg (falls back to default with dev warning) or iconOnly
  ...
  - use asChild to render button markup as an anchor or other element

embedding_hint: >
  button cta action trigger submit primary destructive basic
  outline transparent link click icon left right small large
  hover pressed focused disabled state flat linear asChild anchor
```
...
family_invariants:
  - "Use Flat card only when placed on a White background"
confidence: 0.85
version: 1.1.0
...
when:
  - grouping related content into a visually distinct element
...
family_invariants:
  - "Container: w-auto max-w-xs rounded-lg border-0 bg-card text-card-foreground shadow-md p-4"
  - "Open delay 200ms, close delay 100ms — prevents accidental triggers"
  - "Must be keyboard-accessible via focus"
  - "Interior layout is fully consumer-defined via children"
confidence: 0.9
version: 1.1.0
...
summary: >
  ...
  context without navigating away from the current view. Interior
  layout is open — the block owns only the container shell.

when:
  - previewing a linked entity (provider profile, patient summary, medication detail)
  - showing supplementary info on hover without navigation
  - augmenting a text link, badge, or avatar with richer context
...
props:
  positioning:
    - side: preferred opening side (top | right | bottom | left); subject to collision flip
    - sideOffset: pixel gap between trigger and card
    - align: alignment along the side axis (start | center | end)
    - alignOffset: pixel nudge along the alignment axis
    - avoidCollisions: set false to disable auto-flip (default true)
    - collisionPadding: viewport edge padding before a flip triggers
  state:
    - open / onOpenChange: controlled open state
    - defaultOpen: uncontrolled initial open state
  timing:
    - openDelay: ms before card opens (default 200)
    - closeDelay: ms before card closes after pointer leaves (default 100)

key_rules:
  - never used for critical information (hover unavailable on touch)
  - open delay is intentional — prevents accidental triggers on fast cursor passes
  - close delay allows cursor re-entry into the card without dismissing
  - keyboard accessible via focus on the trigger element
  - interior content layout is the consumer's responsibility — no structure is imposed
...
family_invariants:
  - "Animation: animate-spin, linear — never ease or bounce; duration set by the component"
...
summary: >
  ...
  data fetches, icon-replacement loaders. Three sizes: sm, default, and lg.
...
sizes:
  sm: inside buttons, inline next to text
  default: standalone inline loading regions
  lg: overlay centers, modal loading states
...
key_rules:
  ...
  - never show spinner for operations expected to complete near-instantly — prefer no indicator for instant responses

frozen:
  - "animation: always animate-spin linear — no bounce, no ease, no pulse; duration set by the component"
...
family_invariants:
  ...
  - "Value font size: do not override — set by the component"
  - "Value: font-medium tabular-nums leading-none"
...
frozen:
  - "zones: label and value — both always present; font sizes set by the component, do not override"
...
key_rules:
  - open delay is intentional — prevents accidental triggers on hover
  - close delay allows smooth transitions; set by the component
